    ...
    if process_request.file_id:
        ...
        if asset_record is None:
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST,
                                content={"error":ResponseSignal.FILE_NOT_FOUND.value
                                })
        ...
        project_files_ids={file.id: file.asset_name for file in project_files}
        
    if len(project_files_ids)==0:
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST,content={"error":ResponseSignal.FILE_NOT_FOUND.value})
